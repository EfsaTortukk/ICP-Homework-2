...
fn get_metadata(/* token_id: u64 */) /* -> Result<&'static MetadataDesc> */
{
    ...
    let res: Result<()> = STATE.with(|state| {
        ...
        let metadata = MetadataPart {
            ...
            name: nft.metadata.name.clone(),                // Include NFT name
            description: nft.metadata.description.clone(),  // Include NFT description
            image_url: nft.metadata.image_url.clone(),      // Include NFT image URL
    ...
    STATE.with(|state| {
        ...
        let metadata: Vec<_> = state
            ...
            .map(|n| ExtendedMetadataResult {
                metadata_desc: vec![MetadataPart {
                    purpose: MetadataPurpose::Rendered,
                    name: n.metadata.name.clone(),                // Include NFT name
                    description: n.metadata.description.clone(),  // Include NFT description
                    image_url: n.metadata.image_url.clone(),      // Include NFT image URL
                    key_val_data: HashMap::new(),
                    data: Vec::new(),
                }],
                token_id: n.id,
...
#[update]
fn mint_trading_card(to: Principal, metadata: MetadataDesc, content: Vec<u8>) -> Result<MintResult, ConstrainedError> {
    let (txid, tkid) = STATE.with(|state| {
        let mut state = state.borrow_mut();
        if !state.custodians.contains(&api::caller()) {
            return Err(ConstrainedError::Unauthorized);
...
#[export_name = "canister_query getTradingCardMetadata"]
fn get_trading_card_metadata(token_id: u64) -> Result<MetadataPart, Error> {
    STATE.with(|state| {
        let state = state.borrow();
        let nft = state
            .nfts
